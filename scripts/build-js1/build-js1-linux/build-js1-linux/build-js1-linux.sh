...
# File License:
#     Copyright 2019, Event Store LLP
#
...
function buildv8ProjectionsLibrary() {
    ...
    pushd "$V8_BUILD_DIRECTORY" > /dev/null
        ./tools/dev/v8gen.py -vv x64."$V8_CONFIGURATION" -- v8_monolithic=true v8_use_external_startup_data=false use_sysroot=false use_cxx11=true use_pic=true use_custom_libcxx=true use_custom_libcxx_for_host=true treat_warnings_as_errors=true
        ninja -C "$v8OutputDir"
    popd > /dev/null
    ...
    LIBCXX_LIB_DIR="$v8OutputDir/obj/buildtools/third_party/libc++/libc++/"

    pushd $LIBCXX_LIB_DIR > /dev/null
        ar rvs libc++.a *.o
        ar rvs libc++.a ../../libc++abi/libc++abi/*.o
    popd > /dev/null

    pushd "$EVENTSTORE_DIRECTORY/src/EventStore.Projections.v8Integration/" > /dev/null
        local outputObj=$outputDir/libjs1.so
        "$CLANG_DIR/bin/clang++" -g -I "$V8_INCLUDE_DIR" -I "$LIBCXX_INCLUDE_DIR" *.cpp -m64 -o "$outputObj" -fPIC -shared -std=c++11 -stdlib=libc++ -lc++ -L "$LIBCXX_LIB_DIR" -lv8_monolith -L "$V8_LIB_DIR" -lrt -lpthread
