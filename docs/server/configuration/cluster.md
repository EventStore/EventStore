...
EventStoreDB allows you to run multiple nodes in a cluster for high availability.
...
Use the `ClusterSize` option to tell each cluster node how many nodes the cluster should have.
...
Typical values for the `ClusterSize` setting are three or five (to have a majority of two nodes and a majority of three nodes). We recommended setting this to an odd number of nodes to minimize the chance of a tie during elections, which would lengthen the election process.

## Internal communication

When setting up a cluster, the nodes must be able to reach each other over both the HTTP and the internal TCP channels. Ensure these ports are open on firewalls on the machines and between the machines.
...
The multi-address DNS name cluster discovery only works for clusters that use certificates signed by a private certificate authority, or run insecure. For other scenarios, you need to provide the gossip seed using the hostnames of other cluster nodes.

### Cluster with DNS

When you tell EventStoreDB to use DNS for its gossip, the server will resolve the DNS name to a list of IP addresses and connect to each address to find other nodes. This method is very flexible because you can change the list of nodes on your DNS server without changing the cluster configuration. The DNS method is also useful in automated deployment scenarios when you control both the cluster deployment and the DNS server from your infrastructure-as-code scripts.

To use DNS discovery, you must set the `ClusterDns` option to the DNS name that allows making an HTTP call to it. When the server starts, it will attempt to make a gRPC call using the `https://<cluster-dns>:<gossip-port>` URL (`http` if the cluster is insecure).

When using a certificate signed by a publicly trusted CA, you'd generally use the wildcard certificate. Ensure that the cluster DNS name fits the wildcard, otherwise the request will fail on SSL check.

When using certificates signed by a private CA, the cluster DNS name must be included in each node's certificate as SAN (subject alternative name).

You must also have the `DiscoverViaDns` option set to `true`.  This is its default value.
...
**Default**: `fake.dns`, which doesn't resolve to anything. You have to set it to a proper DNS name when used in combination with the DNS discovery (see next setting).
...
When using DNS for cluster gossip, you might need to set the `GossipPort` setting to the HTTP port if the external node port setting is not set to the `2113` default port. Refer to [gossip port](#gossip-port) option documentation to learn more.
...
The gossip protocol configuration can be changed using the settings listed below. When running in a cloud environment, pay attention to the settings related to time, like intervals and timeouts.

### Gossip port

The gossip port is used to construct the URL to make a gossip request to other nodes discovered via DNS. It is not used when using gossip seeds because, in that case, the list contains IP addresses and the port.

::: warning
Typically, the cluster gossip port is the same as the node port, so you don't need to change this setting.
:::
...
Cluster nodes try to ensure that the communication with their neighbor nodes is not broken. They use the gossip protocol and call each other after a specified period of time. This period is called the gossip interval. You can change the `GossipInvervalMs` setting so cluster nodes check in with each other more or less frequently.
...
A busy node might delay the response when nodes call each other using the gossip protocol to understand the cluster status. When a node is not getting a response from another node, it might consider the other node dead. Such a situation might trigger the election process.

If your cluster network is congested, you might increase the gossip timeout using the `GossipTimeoutMs` setting so nodes will be more tolerant of delayed gossip responses. The default value is 2.5 seconds (2500 ms).
...
The leader elections are separate from the node gossip and are used to elect a node as Leader.

Sometimes, the leader election messages may be delayed, resulting in longer election cycles. In cases where you start seeing election timeouts in the logs or need to increase the gossip timeout due to a congested network, consider increasing the leader election timeout.
...
You can add read-only replica nodes, which will not become cluster members and will not take part in elections. Read-only replicas can be used for scaling up reads if you have many catch-up subscriptions and want to offload cluster members.

A cluster asynchronously replicates data one way to a node with the read-only replica role. The read-only replica node is not part of the cluster, so it does not add to the replication requirements needed to acknowledge a write. For this reason, a node with a read-only replica role does not add much overhead to the other nodes.
...
The replica node needs to have the cluster gossip DNS or seed configured. Use DNS names or IP addresses of all other cluster nodes for the gossip seed, except read-only replicas.

### Node priority

You can control which clones the cluster promotes with the `NodePriority` setting. The default value is `0`, and the cluster is likelier to promote nodes with higher values.
