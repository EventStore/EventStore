...
namespace EventStore.ClientAPI.Tests {
	...
	public class subscribe_to_all : EventStoreClientAPITest, IClassFixture<EventStoreClientAPIFixture>, IAsyncLifetime {
		...
		public async Task concurrently(bool useSsl) {
			...
			using (await connection.SubscribeToAllAsync(false, EventAppeared1, SubscriptionDropped1))
			using (await connection.SubscribeToAllAsync(false, EventAppeared2, SubscriptionDropped2)) {
		...
		public async Task drops_on_subscriber_error(bool useSsl) {
			...
			using var _ = await connection
				.SubscribeToAllAsync(false, EventAppeared, SubscriptionDropped);
