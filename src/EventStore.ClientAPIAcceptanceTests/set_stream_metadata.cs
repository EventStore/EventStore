...
namespace EventStore.ClientAPIAcceptanceTests {
	...
	public class set_stream_metadata : EventStoreClientAPITest, IClassFixture<EventStoreClientAPIFixture> {
		...
		public async Task returns_expected_result(long expectedVersion, string displayName, bool useSsl,
			...
			await connection.SetStreamMetadataAsync(streamName, expectedVersion, expected);

			var result = await connection.GetStreamMetadataAsync(streamName);
