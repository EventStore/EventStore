...
namespace EventStore.Common.Configuration {
	public static class ConfigurationBuilderExtensions {
		...
		public static IConfigurationBuilder AddConfigFiles(
			this IConfigurationBuilder builder,
			string pattern) {

			foreach (var directory in Locations.GetPotentialConfigurationDirectories()) {
				foreach (var configFile in Directory.EnumerateFiles(directory, pattern)) {
					builder.AddConfigFile(configFile, reloadOnChange: true);
				}
			}
