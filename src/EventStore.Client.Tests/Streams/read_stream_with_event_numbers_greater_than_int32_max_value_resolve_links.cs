	...
	public class read_stream_with_event_numbers_greater_than_int32_max_value_resolve_links
		...
		public async Task should_be_able_to_read_linked_stream_forward() {
			var result = await _fixture.Client
				.ReadStreamAsync(Direction.Forwards, LinkedStream, StreamRevision.Start, 100, resolveLinkTos: true)
				.ToArrayAsync();
		...
		public async Task should_be_able_to_read_stream_backward() {
			var result = await _fixture.Client
				.ReadStreamAsync(Direction.Backwards, LinkedStream, new StreamRevision(10), 100, resolveLinkTos: true)
				.Reverse()
				.ToArrayAsync();
		...
		public async Task should_be_able_to_read_all_forward() {
			var result = await _fixture.Client.ReadAllAsync(Direction.Forwards, Position.Start, 100,
					resolveLinkTos: true, userCredentials: TestCredentials.Root)
		...
		public async Task should_be_able_to_read_all_backward() {
			var result = await _fixture.Client.ReadAllAsync(Direction.Backwards, Position.End, 100,
					resolveLinkTos: true, userCredentials: TestCredentials.Root)
				.Where(x => x.OriginalStreamId == LinkedStream)
				.Reverse()
				.ToArrayAsync();
