...
namespace EventStore.TestClient.Commands.Grpc {
	internal class WriteFloodProcessor : ICmdProcessor {
		...
		class SimpleClient : IDisposable {
			...
			private readonly ChannelWriter<BatchAppendReq> _outbound;
			...
			public SimpleClient() {
				...
				var queue = Channel.CreateBounded<BatchAppendReq>(10000);
				var append = _client.BatchAppend();
				_outbound = queue.Writer;
				RunOutbound(queue.Reader, append.RequestStream);
				RunInbound(append.ResponseStream);
			}

			private async void RunInbound(IAsyncStreamReader<BatchAppendResp> responseStream) {
				try {
					await foreach (var response in responseStream.ReadAllAsync()) {
						if (!_pendingRequests.TryRemove(Uuid.FromDto(response.CorrelationId), out var completion)) {
							continue;
						}
						if (response.ResultCase == BatchAppendResp.ResultOneofCase.Success) {
							completion.TrySetResult();
						}
						else if (response.ResultCase == BatchAppendResp.ResultOneofCase.Error) {
							completion.TrySetException(response.Error.Details switch {
								{ } a when a.Is(WrongExpectedVersion.Descriptor) => new Exception("Wrong expected version."),
								_ => new Exception()
							});
						}
					}
			...
			public async Task AppendRequestStreamed(string stream, IEnumerable<EventData> events) {
				...
				_pendingRequests.TryAdd(correlationId, cts);

				var appendReq = new BatchAppendReq {
				...
				foreach (var e in events) {
					appendReq.ProposedMessages.Add(
					...
					if (appendReq.CalculateSize() > 3 * 1024 * 1024) {
						await _outbound.WriteAsync(appendReq);
						appendReq = new BatchAppendReq {
							CorrelationId = correlationIdDto
						};
					}
				}

				appendReq.IsFinal = true;

				await _outbound.WriteAsync(appendReq);
				await cts.Task;
			}

			async void RunOutbound(ChannelReader<BatchAppendReq> channelReader,
				IClientStreamWriter<BatchAppendReq> requestStream) {
				try {
		...
		public sealed class EventData {
			...
			public EventData(Uuid eventId, string type, ReadOnlyMemory<byte> data,
