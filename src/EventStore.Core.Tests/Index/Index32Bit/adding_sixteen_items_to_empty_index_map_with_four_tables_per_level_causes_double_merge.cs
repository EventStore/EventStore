...
namespace EventStore.Core.Tests.Index.Index32Bit
{
    ...
    public class adding_sixteen_items_to_empty_index_map_with_four_tables_per_level_causes_double_merge: SpecificationWithDirectoryPerTestFixture
    {
        ...
        public override void TestFixtureSetUp()
        {
            ...
            _result = _map.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath()), 0, 0, (streamId, hash) => hash, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath()), 0, 0, (streamId, hash) => hash, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath()), 0, 0, (streamId, hash) => hash, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath()), 0, 0, (streamId, hash) => hash, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion);
            ...
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath()), 0, 0, (streamId, hash) => hash, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath()), 1, 2, (streamId, hash) => hash, _ => new System.Tuple<string, bool>("", true), new FakeFilenameProvider(_finalmergefile, _finalmergefile2), _ptableVersion);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
        }
