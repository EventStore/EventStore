...
namespace EventStore.Core.Tests.Index.IndexV1
{
    ...
    public class corrupt_index_should: SpecificationWithDirectoryPerTestFixture
    {
        ...
        private void CorruptPTableFile(string ptableFile, byte version, string corruptionType){
            ...
            using (FileStream stream = File.Open(ptableFile, FileMode.Open))
            {
                if(corruptionType == "footerFileType"){
                    stream.Seek(PTableHeader.Size + numIndexEntries * indexEntrySize + numMidpoints * indexEntrySize,SeekOrigin.Begin);
                    data[0] = 0xFF;
                    stream.Write(data,0,1);
                }
                else if(corruptionType == "footerVersion"){
                    stream.Seek(PTableHeader.Size + numIndexEntries * indexEntrySize + numMidpoints * indexEntrySize + 1,SeekOrigin.Begin);
                    data[0] = 0xFF;
                    stream.Write(data,0,1);
                }
                else if(corruptionType == "negativeIndexEntriesSize"){
                    stream.Seek(PTableHeader.Size + numIndexEntries * indexEntrySize + numMidpoints * indexEntrySize + 2,SeekOrigin.Begin);
                    data[0] = 0xFF;
                    data[1] = 0xFF;
                    data[2] = 0xFF;
                    data[3] = 0xFF;
                    ...
                    var footerPosition = PTableHeader.Size + numIndexEntries * indexEntrySize + numMidpoints * indexEntrySize;
                    ...
                    foreach(int entry in indexEntriesToCorrupt){
                        ...
                        if(version >= PTableVersions.IndexV4){
                            ...
                            stream.Seek(PTableHeader.Size + numIndexEntries * indexEntrySize + entry * indexEntrySize,SeekOrigin.Begin);
                            stream.Write(data,0,indexEntryKeySize);
                        }
                    }
                }
                    ...
                    if(version >= PTableVersions.IndexV4){
                        ...
                        stream.Seek(PTableHeader.Size + numIndexEntries * indexEntrySize + 3 * indexEntrySize - sizeof(long),SeekOrigin.Begin);
        ...
        public void throw_exception_if_index_entries_not_multiple_of_index_entry_size(byte version, bool skipIndexVerify)
        {
        ...
        [TestCase(PTableVersions.IndexV4,false)]
        [TestCase(PTableVersions.IndexV4,true)]
        public void throw_exception_if_midpoints_index_entries_not_in_descending_order(byte version, bool skipIndexVerify)
        {
            string ptableFileName = ConstructPTable(version);
        ...
        [TestCase(PTableVersions.IndexV4,false)]
        [TestCase(PTableVersions.IndexV4,true)]
        public void throw_exception_if_midpoints_item_indexes_not_in_ascending_order(byte version, bool skipIndexVerify)
        {
            string ptableFileName = ConstructPTable(version);
        ...
        [TestCase(PTableVersions.IndexV3,true)]
        [TestCase(PTableVersions.IndexV4,true)]
        public void throw_exception_if_index_entries_not_descending_during_ptable_get_range(byte version, bool skipIndexVerify)
        {
            string ptableFileName = ConstructPTable(version);
            CorruptPTableFile(ptableFileName, version, "zeroOutMiddleEntries");
        ...
        [TestCase(PTableVersions.IndexV3,true)]
        [TestCase(PTableVersions.IndexV4,true)]
        public void throw_exception_if_index_entries_not_descending_during_ptable_get_latest_entry(byte version, bool skipIndexVerify)
        {
            string ptableFileName = ConstructPTable(version);
            CorruptPTableFile(ptableFileName, version, "zeroOutMiddleEntries");
            //loading with a depth of 1 should load only 2 midpoints (first and last index entry)
        ...
        [TestCase(PTableVersions.IndexV3,true)]
        [TestCase(PTableVersions.IndexV4,true)]
        public void throw_exception_if_index_entries_not_descending_during_ptable_get_latest_entry_2(byte version, bool skipIndexVerify)
        {
            string ptableFileName = ConstructPTable(version);
            CorruptPTableFile(ptableFileName, version, "maxOutMiddleEntries");
            //loading with a depth of 1 should load only 2 midpoints (first and last index entry)
        ...
        [TestCase(PTableVersions.IndexV3,true)]
        [TestCase(PTableVersions.IndexV4,true)]
        public void throw_exception_if_index_entries_not_descending_during_ptable_get_oldest_entry(byte version, bool skipIndexVerify)
        {
            string ptableFileName = ConstructPTable(version);
            CorruptPTableFile(ptableFileName, version, "zeroOutMiddleEntries");
            //loading with a depth of 1 should load only 2 midpoints (first and last index entry)
        ...
        [TestCase(PTableVersions.IndexV3,true)]
        [TestCase(PTableVersions.IndexV4,true)]
        public void throw_exception_if_index_entries_not_descending_during_ptable_get_one_value(byte version, bool skipIndexVerify)
        {
            string ptableFileName = ConstructPTable(version);
            CorruptPTableFile(ptableFileName, version, "zeroOutMiddleEntries");
            //loading with a depth of 1 should load only 2 midpoints (first and last index entry)
