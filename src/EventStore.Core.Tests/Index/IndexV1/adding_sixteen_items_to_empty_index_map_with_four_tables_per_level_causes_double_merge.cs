...
namespace EventStore.Core.Tests.Index.IndexV1
{
    ...
    public class adding_sixteen_items_to_empty_index_map_with_four_tables_per_level_causes_double_merge: SpecificationWithDirectoryPerTestFixture
    {
        ...
        public override void TestFixtureSetUp()
        {
            ...
            _result = _map.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath(), false, false), 0, 0, (streamId, hash) => hash, _ => true, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion, false, false);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath(), false, false), 0, 0, (streamId, hash) => hash, _ => true, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion, false, false);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath(), false, false), 0, 0, (streamId, hash) => hash, _ => true, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion, false, false);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath(), false, false), 0, 0, (streamId, hash) => hash, _ => true, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion, false, false);
            ...
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath(), false, false), 0, 0, (streamId, hash) => hash, _ => true, _ => new System.Tuple<string, bool>("", true), guidFilename, _ptableVersion, false, false);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
            _result = _result.MergedMap.AddPTable(PTable.FromMemtable(memtable, GetTempFilePath(), false, false), 1, 2, (streamId, hash) => hash, _ => true, _ => new System.Tuple<string, bool>("", true), new FakeFilenameProvider(_finalmergefile, _finalmergefile2), _ptableVersion, false, false);
            _result.ToDelete.ForEach(x => x.MarkForDestruction());
        }
