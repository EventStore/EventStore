...
namespace EventStore.Core.Tests.Integration.ClientRequests {
	public class when_starting_a_transaction_expecting_no_stream : SpecificationWithRequestManagerIntegration {
		...
		public void successful_request_message_is_publised() {
			AssertEx.IsOrBecomesTrue(() => Produced.ContainsN<StorageMessage.RequestCompleted>(1, x => x.CorrelationId == InternalCorrId && x.Success));
		}
