...
{
    [TestFixture]
    public class when_having_stream_with_truncatebefore_specified_after : ScavengeTestScenario
    {
        protected override DbResult CreateDb(TFChunkDbCreationHelper dbCreator)
        {
            return dbCreator
                .Chunk(Rec.Prepare(1, "bla"), // event 0
                    ...
                    Rec.Commit(3, "bla"),
                    Rec.Prepare(0, "$$bla", metadata: new StreamMetadata(truncateBefore: 7)),
                    Rec.Commit(0, "$$bla")
                )
                .CompleteLastChunk()
                .CreateDb();
        ...
        protected override LogRecord[][] KeptRecords(DbResult dbResult)
        {
            return new[]
            {
                dbResult.Recs[0].Where((x, i) => new [] {9, 10, 11, 12, 13, 14}.Contains(i)).ToArray()
            };
        }
