...
namespace EventStore.Core.Tests.TransactionLog
{
    ...
    public class commit_log_record_should
    {
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => { new CommitLogRecord(-1, Guid.Empty, 0, DateTime.UtcNow, 0); });
        }
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => { new CommitLogRecord(0, Guid.NewGuid(), -1, DateTime.UtcNow, 0); });
        }

        [Test]
        public void throw_argumentoutofrangeexception_when_given_negative_eventversion()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => { new CommitLogRecord(0, Guid.NewGuid(), 0, DateTime.UtcNow, -1); });
        }
    }
}