...
namespace EventStore.Core.Tests.Services.Storage.Scavenge
{
    ...
    public class when_scavenging_tfchunk_with_version0_log_records_using_transactions : ReadIndexTestScenario
    {
        ...
        protected override void WriteTestScenario()
        {
            ...
            _t2CommitPos = WriteCommitV0(t2.CorrelationId, WriterCheckpoint.ReadNonFlushed(), t2.TransactionPosition, t2.EventStreamId, 0, out _postCommitPos);
            _t1CommitPos = WriteCommitV0(t1.CorrelationId, WriterCheckpoint.ReadNonFlushed(), t1.TransactionPosition, t1.EventStreamId, 0, out _postCommitPos);

            Writer.CompleteChunk();

            // Need to have a second chunk as otherwise the checkpoints will be off
            _random1 = WriteSingleEventWithLogVersion0(Guid.NewGuid(), "random-stream", WriterCheckpoint.ReadNonFlushed(), 0);
        ...
        private long WriteCommitV0(Guid correlationId, long logPosition, long transactionPosition, string eventStreamId, long eventNumber, out long pos)
        {
        ...
        [Test]
        public void read_all_events_forward_returns_no_transaction_records_when_prepare_position_is_greater_than_last_prepare_in_commit()
        {
        ...
        {
            var res1 = ReadIndex.ReadAllEventsForward(new TFPos(_t2CommitPos, _p4.LogPosition), 10); // end of first commit
