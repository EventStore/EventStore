...
namespace EventStore.Core.Tests.Services.Storage.Scavenge
{
    ...
    [TestFixture]
    public class when_creating_a_db_with_event_numbers_greater_than_int_maxvalue : ReadIndexTestScenario
    {
        // private EventRecord _event7;
        // private PrepareLogRecord _event7prepare;
        // private CommitLogRecord _event7commit;
        ...
        protected override void WriteTestScenario()
        {
            WriteSingleEvent("ES", intMaxValue + 1, new string('.', 3000)); // chunk 1
            WriteSingleEvent("ES", intMaxValue + 2, new string('.', 3000));
            WriteSingleEvent("ES", intMaxValue + 3, new string('.', 3000));

            WriteSingleEvent("ES", intMaxValue + 4, new string('.', 3000), retryOnFail: true); // chunk 2
            WriteSingleEvent("ES", intMaxValue + 5, new string('.', 3000));
            // Scavenge(completeLast: false, mergeChunks: false);
        }

        [Test]
        public void path_is_not_null()
        {
            Console.WriteLine("DB Path: {0}", Db.Config.Path);
            Assert.IsNotNull(Db.Config.Path);
        }
        // [Test]
        // public void read_all_forward_does_not_return_scavenged_deleted_stream_events_and_return_remaining()
        // {
