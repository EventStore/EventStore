	...
	public class ClusterSettingsFactory {
		...
		private static ClusterVNodeSettings CreateVNode(int nodeNumber, bool isReadOnlyReplica) {
			...
			var vnode = new ClusterVNodeSettings(Guid.NewGuid(), 0,
				...
				new Data.GossipAdvertiseInfo(GetLoopbackForPort(tcpIntPort).ToEventStoreEndPoint(), null,
					GetLoopbackForPort(tcpExtPort).ToEventStoreEndPoint(), null,
					GetLoopbackForPort(httpIntPort).ToEventStoreEndPoint(),
					GetLoopbackForPort(httpExtPort).ToEventStoreEndPoint(),
					null, null, 0, 0),
