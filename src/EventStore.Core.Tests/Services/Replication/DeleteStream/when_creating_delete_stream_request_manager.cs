...
namespace EventStore.Core.Tests.Services.Replication.DeleteStream
{
    ...
    public class when_creating_delete_stream_request_manager
    {
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new DeleteStreamTwoPhaseRequestManager(null, 3, PrepareTimeout, CommitTimeout, false));
        }
        ...
        public void zero_prepare_ack_count_throws_argument_out_range()
        {
            Assert.Throws<ArgumentOutOfRangeException>(
                () => new DeleteStreamTwoPhaseRequestManager(new FakePublisher(), 0, PrepareTimeout, CommitTimeout, false));
        }
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(
                () => new DeleteStreamTwoPhaseRequestManager(new FakePublisher(), -1, PrepareTimeout, CommitTimeout, false));
        }
    }
}
