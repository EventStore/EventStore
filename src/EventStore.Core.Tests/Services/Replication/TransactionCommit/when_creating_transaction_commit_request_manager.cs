...
namespace EventStore.Core.Tests.Services.Replication.TransactionCommit
{
    ...
    public class when_creating_transaction_commit_request_manager
    {
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new TransactionCommitTwoPhaseRequestManager(null, 3, PrepareTimeout, CommitTimeout, false));
        }
        ...
        public void zero_prepare_ack_count_throws_argument_out_range()
        {
            Assert.Throws<ArgumentOutOfRangeException>(
                () => new TransactionCommitTwoPhaseRequestManager(new FakePublisher(), 0, PrepareTimeout, CommitTimeout, false));
        }
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(
                () => new TransactionCommitTwoPhaseRequestManager(new FakePublisher(), -1, PrepareTimeout, CommitTimeout, false));
        }
    }
}