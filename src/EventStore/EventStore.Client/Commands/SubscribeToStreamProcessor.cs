    ...
    internal class SubscribeToStreamProcessor : ICmdProcessor
    {
        ...
        public bool Execute(CommandProcessorContext context, string[] args)
        {
            ...
            if (args.Length == 0)
            {
                ...
                var cmd = new ClientMessageDto.SubscribeToAllStreams(Guid.Empty);
                connection.EnqueueSend(new TcpPackage(TcpCommand.SubscribeToAllStreams, cmd.Serialize()).AsByteArray());
            }
            else
            {
                foreach (var stream in args)
                {
                    ...
                    var cmd = new ClientMessageDto.SubscribeToStream(Guid.Empty, stream);
                    connection.EnqueueSend(new TcpPackage(TcpCommand.SubscribeToStream, cmd.Serialize()).AsByteArray());
                }
            }
