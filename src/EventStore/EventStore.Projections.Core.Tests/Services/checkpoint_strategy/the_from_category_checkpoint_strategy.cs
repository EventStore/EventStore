...
namespace EventStore.Projections.Core.Tests.Services.checkpoint_strategy
{
    ...
    public class the_from_category_checkpoint_strategy
    {
        ...
        {
            Assert.AreEqual(true, _strategy.IsCheckpointTagAfterEventPosition(CheckpointTag.FromStreamPosition("$ce-category1", 100, 150), new EventPosition(100, 30)));
        }

        [Test]
        public void can_compare_checkpoint_tag_with_event_position_negative()
        {
            Assert.AreEqual(false, _strategy.IsCheckpointTagAfterEventPosition(CheckpointTag.FromStreamPosition("$ce-category1", 100, 90), new EventPosition(100, 30)));
        }
