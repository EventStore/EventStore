...
namespace EventStore.Core.Tests.Services.Transport.Http
{
    ...
    public abstract class uri_router_should
    {
        ...
        {
            Assert.That(() => 
                _router.RegisterAction(new ControllerAction("/halt", HttpMethod.Get, Codec.NoCodecs, FakeController.SupportedCodecs), (x, y) => new RequestParams(TimeSpan.Zero)),
        ...
        public void match_greedy_route_in_the_root_to_any_path()
        {
            ...
            tmpRouter.RegisterAction(new ControllerAction("/{*greedy}", HttpMethod.Get, Codec.NoCodecs, FakeController.SupportedCodecs), (x, y) => new RequestParams(TimeSpan.Zero));
