...
public class PersistentSubscriptionLastCheckpointedEventCommitPositionMetricTests : IDisposable {
	private readonly Disposables _disposables = new();
	private readonly List<MonitoringMessage.PersistentSubscriptionInfo> _fakeStatistics = [];

	public PersistentSubscriptionLastCheckpointedEventCommitPositionMetricTests() {
	...
	private (PersistentSubscriptionLastCheckpointedEventCommitPositionMetric, TestMeterListener<long>) GenSut(
		...
		var sut = new PersistentSubscriptionLastCheckpointedEventCommitPositionMetric(meter, "test-metric");
	...
	public void observe_metrics_data() {
		...
		AssertMeasurements(listener,
			AssertMeasurement("$all", "testGroup", 211845)
		);
	}
