...
public class PersistentSubscriptionInFlightMessagesMetricTests : IDisposable {
	private readonly Disposables _disposables = new();
	private readonly List<MonitoringMessage.PersistentSubscriptionInfo> _fakeStatistics = [];

	public PersistentSubscriptionInFlightMessagesMetricTests() {
	...
	private (PersistentSubscriptionInFlightMessagesMetric, TestMeterListener<long>) GenSut(
		...
		var sut = new PersistentSubscriptionInFlightMessagesMetric(meter, "test-metric");
	...
	public void observe_metrics_data() {
		...
		AssertMeasurements(listener,
			...
			AssertMeasurement("$all", "testGroup", 5)
		);
	}
