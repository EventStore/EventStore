...
public class PersistentSubscriptionOldestParkedMessageMetricTests : IDisposable {
	private readonly Disposables _disposables = new();
	private readonly List<MonitoringMessage.PersistentSubscriptionInfo> _fakeStatistics = [];

	public PersistentSubscriptionOldestParkedMessageMetricTests() {
	...
	private (PersistentSubscriptionOldestParkedMessageMetric, TestMeterListener<long>) GenSut(
		...
		var sut = new PersistentSubscriptionOldestParkedMessageMetric(meter, "test-metric");
	...
	public void observe_metrics_data() {
		...
		AssertMeasurements(listener,
			AssertMeasurement("test", "testGroup", 50),
			AssertMeasurement("$all", "testGroup", 50)
		);
	}
