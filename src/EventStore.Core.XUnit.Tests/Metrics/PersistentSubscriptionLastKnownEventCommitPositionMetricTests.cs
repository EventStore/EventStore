...
public class PersistentSubscriptionLastKnownEventCommitPositionMetricTests : IDisposable {
	private readonly Disposables _disposables = new();
	private readonly List<MonitoringMessage.PersistentSubscriptionInfo> _fakeStatistics = [];

	public PersistentSubscriptionLastKnownEventCommitPositionMetricTests() {
	...
	private (PersistentSubscriptionLastKnownEventCommitPositionMetric, TestMeterListener<long>) GenSut(
		...
		var sut = new PersistentSubscriptionLastKnownEventCommitPositionMetric(meter, "test-metric");
	...
	public void observe_metrics_data() {
		...
		AssertMeasurements(listener,
			AssertMeasurement("$all", "testGroup", 211200));
	}
