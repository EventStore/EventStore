...
public class PersistentSubscriptionConnectionCountMetricTests : IDisposable {
	private readonly Disposables _disposables = new();
	private readonly List<MonitoringMessage.PersistentSubscriptionInfo> _fakeStatistics = [];

	public PersistentSubscriptionConnectionCountMetricTests() {
	...
	private (PersistentSubscriptionConnectionCountMetric, TestMeterListener<long>) GenSut(
		...
		var sut = new PersistentSubscriptionConnectionCountMetric(meter, "test-metric");
	...
	public void observe_metrics_data() {
		...
		AssertMeasurements(listener,
			AssertMeasurement("test", "testGroup", 0),
			AssertMeasurement("$all", "testGroup", 0)
		);
	}
