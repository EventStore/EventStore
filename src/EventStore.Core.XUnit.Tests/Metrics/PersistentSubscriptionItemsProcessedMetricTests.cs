...
public class PersistentSubscriptionItemsProcessedMetricTests : IDisposable {
	private readonly Disposables _disposables = new();
	private readonly List<MonitoringMessage.PersistentSubscriptionInfo> _fakeStatistics = [];

	public PersistentSubscriptionItemsProcessedMetricTests() {
	...
	private (PersistentSubscriptionItemsProcessedMetric, TestMeterListener<long>) GenSut(
		...
		var sut = new PersistentSubscriptionItemsProcessedMetric(meter, "test-metric");
	...
	public void observe_metrics_data() {
		...
		AssertMeasurements(listener,
			AssertMeasurement("test", "testGroup", 30),
			AssertMeasurement("$all", "testGroup", 30)
		);
	}
