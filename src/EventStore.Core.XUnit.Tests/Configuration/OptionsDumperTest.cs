...
// 		});
// 		var printable = OptionsDumper.GetPrintableOptions(options.ConfigurationRoot);
//
...
// 		Assert.Equal(nameof(EventStoreCommandLineConfigurationProvider), commandLine.Source.Name);
//
// 		Assert.True(printable.TryGetValue(nameof(TestOptions.SecondSection.FlagOption), out var envVar));
// 		Assert.Equal(nameof(EventStoreEnvironmentVariablesSource), envVar.Source.Name);
//
