...
public class AutoScavengePlugin() : SubsystemsPlugin(name: "auto-scavenge"), IConnectedSubsystemsPlugin {
	const string PublicKey = "";
	...
	public override (bool Enabled, string EnableInstructions) IsEnabled(IConfiguration configuration) {
		var enabledOption = configuration.GetValue<bool?>($"EventStore:AutoScavenge:Enabled");
		var devMode = configuration.GetValue($"EventStore:Dev", defaultValue: false);
		var memDb = configuration.GetValue($"EventStore:MemDb", defaultValue: false);

		// enabled by default
		bool enabled = enabledOption ?? true;

		if (!enabled)
			return (false, "To enable AutoScavenge: Do not set 'EventStore:AutoScavenge:Enabled' to 'false'");
	...
	public override void ConfigureServices(IServiceCollection services, IConfiguration configuration) {
		_options = configuration.GetSection("EventStore").Get<EventStoreOptions>() ?? _options;
	...
	void OnLicenseError(Exception ex) {
	...
	public override void ConfigureApplication(IApplicationBuilder builder, IConfiguration configuration) {
		...
		_ = LicenseMonitor.MonitorAsync(
			featureName: Name,
			requiredEntitlements: ["AUTO_SCAVENGE"],
			licenseService: licenseService,
			onLicenseException: OnLicenseError,
			logger: logger,
