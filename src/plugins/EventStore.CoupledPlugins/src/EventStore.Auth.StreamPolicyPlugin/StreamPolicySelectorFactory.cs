...
public class StreamPolicySelectorFactory : Plugin, IPolicySelectorFactory, ISubsystem {
	...
	private void OnLicenseError(Exception ex) {
		Logger.Information("Stream Policies plugin is not licensed, stream policy authorization may not be enabled.");
		_licensed = false;
	}
	...
	public override (bool Enabled, string EnableInstructions) IsEnabled(IConfiguration configuration) {
		return (_enabled,
			$"Set 'streamAccessPolicyType' in the '{SystemStreams.AuthorizationPolicyRegistryStream}' stream to '{CommandLineName}' to enable stream policies");
	}

	public override void ConfigureApplication(IApplicationBuilder builder, IConfiguration configuration) {
		base.ConfigureApplication(builder, configuration);
