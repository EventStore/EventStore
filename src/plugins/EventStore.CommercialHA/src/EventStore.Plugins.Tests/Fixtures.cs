...
public static class Fixtures {
	...
	// The jwts should be generated occasionally as necessary and then hard coded above.
	// This code can generate the jwts. Adjust the keys as necessary, they will normally
	// need to be the key pair from the actual plugins and licencing plugin.
	// circumventing the license mechanism is against the ESLv2 software license
	public static async Task<string> CreateHardCodedTokensSource() {
		var entitlements = new[] {
			"AUTO_SCAVENGE",
			"ENCRYPTION_AT_REST",
			"LDAPS_AUTHENTICATION",
			"LEGACY_AUTHORIZATION_WITH_STREAM_AUTHORIZATION_DISABLED",
			"LOGS_ENDPOINT",
			"MD5",
			"OAUTH_AUTHENTICATION",
			"OTLP_EXPORTER",
			"STREAM_POLICY_AUTHORIZATION",
			"TCP_CLIENT_PLUGIN",
			"USER_CERTIFICATES",
		...
		sb.AppendLine("static readonly Dictionary<string, string> _hardCodedTokens = new() {");

		foreach (var entitlement in entitlements) {
			var token = await CreateTokenAsync(entitlement);
			sb.AppendLine($"\t\t{{ \"{entitlement}\", \"{token}\" }},");
		}
	...
	private static async Task<string> CreateTokenAsync(string entitlement) {
		...
		var token = new JsonWebTokenHandler().CreateToken(new SecurityTokenDescriptor {
			...
			Audience = "esdb",
			Issuer = "esdb",
			Expires = DateTime.UtcNow + TimeSpan.FromDays(365 * 3),
			Claims = new Dictionary<string, object> {
				...
				{ "IsFloating", $"{true}" },
				{ "DaysRemaining", $"{1}" },
				{ "StartDate", $"{new DateTimeOffset(new DateTime(2024, 4, 26))}" },
				{ entitlement, "true" },
			},
		});

		var license = new License(new JsonWebToken(token));
		if (!await license.ValidateAsync(publicKey))
			throw new Exception("Could not generate a valid license");

		return license.Token.EncodedToken;
	}
