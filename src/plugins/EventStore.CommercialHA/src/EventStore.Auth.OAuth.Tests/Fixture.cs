...
internal class Fixture : IDisposable {
	private const string PluginConfigurationPath = "/etc/eventstore/oauth.conf";
	...
	private static string LicenseInjectorSourceDirectory => Path.Combine(SourceDirectory, "EventStore.Plugins.LicenseInjector");

	private static string PluginPublishDirectory =>
		Path.Combine(BuildDirectory, "EventStore.Auth.OAuth", "net8.0", "publish");

	private static string LicenseInjectorPublishDirectory =>
		Path.Combine(BuildDirectory, "EventStore.Plugins.LicenseInjector", "net8.0", "publish");

	private static string ESImage {
		get {
			var imageWithTag = Environment.GetEnvironmentVariable("ES_IMAGE");
	...
	private Fixture(ITestOutputHelper output, params string[] env) {
		var defaultEnv = new[] {
			"EVENTSTORE_CERTIFICATE_FILE=/opt/eventstore/certs/test.crt",
			"EVENTSTORE_CERTIFICATE_PRIVATE_KEY_FILE=/opt/eventstore/certs/test.key",
			"EVENTSTORE_TRUSTED_ROOT_CERTIFICATES_PATH=/opt/eventstore/certs/",
			$"EVENTSTORE_AUTHENTICATION_CONFIG={PluginConfigurationPath}",
	...
	private async ValueTask Start() {
		...
		await PublishPlugin(LicenseInjectorSourceDirectory);
		...
		var imageName = "eventstore-for-oauth-plugins-unit-tests";
		...
		using var x = new Builder()
			...
			.Run("mv /opt/eventstore/plugins /opt/eventstore/all-plugins")
			// reinstate the tcp plugin
			.Run("mkdir -p /opt/eventstore/plugins")
			.Run("cp -r /opt/eventstore/all-plugins/EventStore.TcpPlugin /opt/eventstore/plugins/EventStore.TcpPlugin")
			.Build();
		...
		_eventStore = new Builder()
			...
			.Mount(PluginPublishDirectory, "/opt/eventstore/plugins/oauth", MountType.ReadOnly)
			.Mount(LicenseInjectorPublishDirectory, "/opt/eventstore/plugins/injector", MountType.ReadOnly)
			.Mount(CertificateDirectory, "/opt/eventstore/certs", MountType.ReadOnly)
