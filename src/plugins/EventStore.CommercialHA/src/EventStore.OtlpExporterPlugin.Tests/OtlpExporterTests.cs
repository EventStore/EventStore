...
public class OtlpExporterTests {
	...
	public async Task respects_expected_scrape_interval() {
		await using var _ = await CreateServer(new(), new Dictionary<string, string?> {
			{ "EventStore:OpenTelemetry:Otlp:Endpoint", Endpoint },
			{ "EventStore:Metrics:ExpectedScrapeIntervalSeconds", "5" },
		});
	...
	public async Task export_interval_superceeds_expected_scrape_interval() {
		await using var _ = await CreateServer(new(), new Dictionary<string, string?> {
			{ "EventStore:OpenTelemetry:Otlp:Endpoint", Endpoint },
			{ "EventStore:OpenTelemetry:Metrics:PeriodicExportingMetricReaderOptions:ExportIntervalMilliseconds", "5000" },
			{ "EventStore:Metrics:ExpectedScrapeIntervalSeconds", "4" },
		});
		Assert.Collection(_logger.LogMessages,
			log => Assert.Equal(
				...
				"(5000 ms) does not match EventStore:Metrics:ExpectedScrapeIntervalSeconds " +
	...
	public async Task exports_metrics() {
		...
		await using var _ = await CreateServer(tcs, new Dictionary<string, string?> {
			{ "EventStore:OpenTelemetry:Otlp:Endpoint", Endpoint },
			{ "EventStore:Metrics:ExpectedScrapeIntervalSeconds", "1" }
		});
