...
namespace EventStore.ClientAPI.Internal {
	internal class StaticEndPointDiscoverer : IEndPointDiscoverer {
		...
		public StaticEndPointDiscoverer(IPEndPoint endPoint, bool isSsl, string targetHost) {
			Ensure.NotNull(endPoint, "endPoint");
			_task = Task.Factory.StartNew(() => new NodeEndPoints(isSsl ? null : endPoint, isSsl ? endPoint : null, targetHost));
		}
