...
public static class DuckDb {
	...
	public static IReadOnlyList<IndexEntry> GetRange(string streamName, ulong streamId, long fromEventNumber, int maxCount) {
		...
		const string query = "select event_number, log_position from idx_all inner join streams on idx_all.stream=streams.id where streams.name=$stream and event_number>=$start order by event_number limit $count";
