...
public sealed class ClusterVNodeController<TStreamId> : ClusterVNodeController, IPublisher, ISubscriber, IEnvelope {
	...
	public ClusterVNodeController(QueueStatsManager statsManager,
		...
		_scheduler = new QueuedHandlerThreadPool(_fsm, "MainQueue", statsManager,
			trackers);
	...
	private VNodeFSM CreateFSM() => new VNodeFSMBuilder(new(this, in _state))
			.InAnyState()
			...
			.WhenOther().ForwardTo(_dispatcher)
			...
			.When<ClientMessage.ReadEvent>().ForwardTo(_dispatcher)
			.When<ClientMessage.ReadStreamEventsForward>().ForwardTo(_dispatcher)
			.When<ClientMessage.ReadStreamEventsBackward>().ForwardTo(_dispatcher)
			.When<ClientMessage.ReadAllEventsForward>().ForwardTo(_dispatcher)
			.When<ClientMessage.ReadAllEventsBackward>().ForwardTo(_dispatcher)
			.When<ClientMessage.FilteredReadAllEventsForward>().ForwardTo(_dispatcher)
			.When<ClientMessage.FilteredReadAllEventsBackward>().ForwardTo(_dispatcher)
			.When<ClientMessage.WriteEvents>().ForwardTo(_dispatcher)
			.When<ClientMessage.TransactionStart>().ForwardTo(_dispatcher)
			.When<ClientMessage.TransactionWrite>().ForwardTo(_dispatcher)
			.When<ClientMessage.TransactionCommit>().ForwardTo(_dispatcher)
			.When<ClientMessage.DeleteStream>().ForwardTo(_dispatcher)
			.When<ClientMessage.CreatePersistentSubscriptionToStream>().ForwardTo(_dispatcher)
			.When<ClientMessage.ConnectToPersistentSubscriptionToStream>().ForwardTo(_dispatcher)
			.When<ClientMessage.UpdatePersistentSubscriptionToStream>().ForwardTo(_dispatcher)
			.When<ClientMessage.DeletePersistentSubscriptionToStream>().ForwardTo(_dispatcher)
			.When<ClientMessage.CreatePersistentSubscriptionToAll>().ForwardTo(_dispatcher)
			.When<ClientMessage.ConnectToPersistentSubscriptionToAll>().ForwardTo(_dispatcher)
			.When<ClientMessage.UpdatePersistentSubscriptionToAll>().ForwardTo(_dispatcher)
			.When<ClientMessage.DeletePersistentSubscriptionToAll>().ForwardTo(_dispatcher)
			...
			.InAnyState()
			.When<ClientMessage.NotHandled>().ForwardTo(_dispatcher)
			.When<ClientMessage.ReadEventCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.ReadStreamEventsForwardCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.ReadStreamEventsBackwardCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.ReadAllEventsForwardCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.ReadAllEventsBackwardCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.FilteredReadAllEventsForwardCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.FilteredReadAllEventsBackwardCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.WriteEventsCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.TransactionStartCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.TransactionWriteCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.TransactionCommitCompleted>().ForwardTo(_dispatcher)
			.When<ClientMessage.DeleteStreamCompleted>().ForwardTo(_dispatcher)
			...
			.When<ReplicationMessage.AckLogPosition>().ForwardTo(_dispatcher)
			.WhenOther().ForwardTo(_dispatcher)
			...
			.When<StorageMessage.WritePrepares>().ForwardTo(_dispatcher)
			.When<StorageMessage.WriteDelete>().ForwardTo(_dispatcher)
			.When<StorageMessage.WriteTransactionStart>().ForwardTo(_dispatcher)
			.When<StorageMessage.WriteTransactionData>().ForwardTo(_dispatcher)
			.When<StorageMessage.WriteTransactionEnd>().ForwardTo(_dispatcher)
			.When<StorageMessage.WriteCommit>().ForwardTo(_dispatcher)
			.WhenOther().ForwardTo(_dispatcher)
			.InAllStatesExcept(VNodeState.Leader, VNodeState.ResigningLeader)
	...
	public void Publish(Message message) => _scheduler.Publish(message);
	...
	private ValueTask Handle(SystemMessage.SystemInit message, CancellationToken token) {
		...
		return _dispatcher.DispatchAsync(message, token);
	}
	...
	private ValueTask Handle(SystemMessage.SystemCoreReady message, CancellationToken token) {
		...
		return _dispatcher.DispatchAsync(msg, token);
	}
	...
	private ValueTask Handle(SystemMessage.SubSystemInitialized message, CancellationToken token) {
		...
		return Interlocked.Decrement(ref _subSystemInitsToExpect) is 0
			? _dispatcher.DispatchAsync(new SystemMessage.SystemReady(), token)
	...
	private ValueTask ForwardRequest(ClientMessage.WriteRequestMessage msg, Message timeoutMessage, CancellationToken token) {
		...
		return _dispatcher.DispatchAsync(new ClientMessage.TcpForwardMessage(msg), token);
	}
	...
	private ValueTask Handle(SystemMessage.WaitForChaserToCatchUp message, CancellationToken token) {
		return message.CorrelationId == _stateCorrelationId
			? _dispatcher.DispatchAsync(message, token)
			: ValueTask.CompletedTask;
	}
	...
	private ValueTask Handle(ReplicationMessage.LeaderConnectionFailed message, CancellationToken token) {
		...
		return _dispatcher.DispatchAsync(TimerMessage.Schedule.Create(LeaderSubscriptionTimeout, this, msg), token);
	}
