	...
	public class VNodeFSM : IHandle<Message> {
		...
		internal VNodeFSM(Func<VNodeState> getState,
			...
			for (int i = 0; i < _handlers.Length; ++i) {
				_handlers[i] = new Action<VNodeState, Message>[MessageHierarchyResolver.MaxMsgTypeId + 1];
				if (handlers[i] != null) {
					foreach (var handler in handlers[i]) {
						_handlers[i][MessageHierarchyResolver.MsgTypeIdByType[handler.Key]] = handler.Value;
					}
				}
			}
		...
		public void Handle(Message message) {
			...
			var parents = MessageHierarchyResolver.ParentsByTypeId[message.MsgTypeId];
