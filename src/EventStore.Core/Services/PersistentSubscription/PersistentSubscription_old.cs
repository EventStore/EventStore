...
{
    public class PersistentSubscription_old
    {
        private const double _minimumPercentageOfFreeSlotsForSwitchingToPushMode = 0.5;

        private static readonly ILogger Log = LogManager.GetLoggerFor<PersistentSubscription_old>();
        ...
        private Dictionary<Guid, PersistentSubscriptionClient_old> _clients = new Dictionary<Guid, PersistentSubscriptionClient_old>();
        ...
        public PersistentSubscription_old(bool resolveLinkTos, 
        ...
        public void InitAsNew()
        {
            ...
            _clients = new Dictionary<Guid, PersistentSubscriptionClient_old>();
        ...
        {
            var client = new PersistentSubscriptionClient_old(correlationId, connectionId, envelope, maxInFlight, user, @from, _totalTimeWatch, _trackLatency);
        ...
        {
            PersistentSubscriptionClient_old clientOld;
            if (_clients.TryGetValue(correlationId, out clientOld))
            {
                _clients.Remove(clientOld.CorrelationId);
                if (sendDropNotification)
                {
                    clientOld.SendDropNotification();
                }
                var unconfirmedEvents = clientOld.GetUnconfirmedEvents();
                Log.Debug("Client {0} disconnected. Rerouting unconfirmed events.", clientOld.ConnectionId);
                foreach (var evnt in unconfirmedEvents)
                {
                    HandleUnhandledEvent(evnt);
                }
        ...
        public void AcknowledgeMessagesProcessed(Guid correlationId, Guid[] processedEventIds)
        {
            ...
            if (_clients.TryGetValue(correlationId, out clientOld))
            {
                var processedEvents = clientOld.ConfirmProcessing(processedEventIds);
        ...
        {
            PersistentSubscriptionClient_old leastBusy = null;
            foreach ( var client in _clients.Values)
            {
        ...
        private void PushEvent(ResolvedEvent resolvedEvent, PersistentSubscriptionClient_old leastBusy)
        {
