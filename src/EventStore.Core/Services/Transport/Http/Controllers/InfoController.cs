...
public static class InfoEndpoints {
	public static void MapGetInfo(this IEndpointRouteBuilder app) {
		...
		app.MapGet("/info/options", (HttpContext ctx, [FromServices] InfoController controller) => {
			if (!ctx.User.LegacyRoleCheck(SystemRoles.Operations) &&
			    !ctx.User.LegacyRoleCheck(SystemRoles.Admins)) {
				return Results.Unauthorized();
			}

			return Results.Ok(controller.GetOptions());
		});
	}
}
...
public class InfoController(
	...
	IAuthenticationProvider authenticationProvider) : IHandle<SystemMessage.StateChangeMessage> {
	private static readonly ILogger Log = Serilog.Log.ForContext<InfoController>();

	internal VNodeState State { get; private set; }
	...
	public void Handle(SystemMessage.StateChangeMessage message) => State = message.State;
	...
	internal IEnumerable<OptionStructure> GetOptions() {
		return options.LoadedOptions.Values.Select(
