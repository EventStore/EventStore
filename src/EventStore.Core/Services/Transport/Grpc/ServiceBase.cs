...
namespace EventStore.Core.Services.Transport.Grpc {
	public class ServiceBase {
		protected static bool GetRequiresLeader(Metadata requestHeaders) {
			var requiresLeaderHeaderValue = requestHeaders.FirstOrDefault(x => x.Key == Constants.Headers.RequiresLeader)?.Value;
			if (string.IsNullOrEmpty(requiresLeaderHeaderValue)) return false;
			return bool.TryParse(requiresLeaderHeaderValue, out var requiresLeader) && requiresLeader;
		}
	}
}
