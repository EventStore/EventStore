...
internal partial class PersistentSubscriptions {
	...
	public override async Task<Empty> RestartSubsystem(Empty request, ServerCallContext context) {
		...
		_publisher.Publish(new PersistentSubscriptionsRestart(new CallbackEnvelope(HandleRestartSubsystemCompleted)));
		...
		void HandleRestartSubsystemCompleted(Message message) {
			...
			switch (message) {
				...
				default:
					restartSubsystemSource.TrySetException(
						RpcExceptions.UnknownMessage<PersistentSubscriptionsRestarting>(message));
					return;
			}
		}
	}
