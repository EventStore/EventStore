...
namespace EventStore.Core.Services.TimerService {
	public sealed class ThreadBasedScheduler : IMonitoredQueue, IScheduler {
		...
		public void Schedule(TimeSpan after, Action<IScheduler, object> callback, object state) {
			var dueTime = _timeProvider.Now.Add(after);
			_pending.Enqueue(new ScheduledTask(dueTime, callback, state));

			if (dueTime.Ticks < Interlocked.Read(ref _nextWakeupTimeTicks))
			...
			while (!_stop) {
				try {
					...
					if (processed == 0) {
						...
						var nextWakeupTimeTicks = _timeProvider.Now.Add(timeout).Ticks;
						Interlocked.Exchange(ref _nextWakeupTimeTicks, nextWakeupTimeTicks);
