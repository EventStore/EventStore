...
public static class RequestClient {
	public static async Task<TResponse> RequestAsync<TRequest, TResponse>(IPublisher publisher, Func<IEnvelope, TRequest> getRequest)
		...
		var response = await channel.Reader.ReadAsync().ConfigureAwait(false);
