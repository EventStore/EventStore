...
namespace EventStore.Core.Bus {
	...
	public class InMemoryBus2 : IBus, ISubscriber, IPublisher, IHandle<Message> {
		...
		public void Subscribe<T>(IHandle<T> handler) where T : Message {
			lock (_handlersLock) {
				...
				if (!MessageHierarchyResolver.Descendants.TryGetValue(typeof(T), out descendants))
	...
	public class InMemoryBus : IBus, ISubscriber, IPublisher, IHandle<Message> {
		...
		public InMemoryBus(string name, bool watchSlowMsg = true, TimeSpan? slowMsgThreshold = null) {
			...
			_handlers = new List<IMessageHandler>[MessageHierarchyResolver.MaxMsgTypeId + 1];
		...
		public void Subscribe<T>(IHandle<T> handler) where T : Message {
			lock (_handlersLock) {
				...
				int[] descendants = MessageHierarchyResolver.DescendantsByType[typeof(T)];
