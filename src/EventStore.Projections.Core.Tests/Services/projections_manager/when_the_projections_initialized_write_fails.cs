...
namespace EventStore.Projections.Core.Tests.Services.projections_manager
{
    ...
    public class when_writing_the_projections_initialized_event_fails : TestFixtureWithProjectionCoreAndManagementServices
    {
        ...
        {
            int retryCount = 0;
            var projectionsInitializedWrite = _consumer.HandledMessages.OfType<ClientMessage.WriteEvents>().Where(x => x.EventStreamId == ProjectionNamesBuilder.ProjectionsRegistrationStream && x.Events[0].EventType == ProjectionEventTypes.ProjectionsInitialized).Last();
            ...
            while (retryCount < 5)
            {
                ...
                _queue.Process();
                projectionsInitializedWrite = _consumer.HandledMessages.OfType<ClientMessage.WriteEvents>().Where(x => x.EventStreamId == ProjectionNamesBuilder.ProjectionsRegistrationStream && x.Events[0].EventType == ProjectionEventTypes.ProjectionsInitialized).LastOrDefault();
