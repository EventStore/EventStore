...
namespace positional_writes
{
    internal sealed class NativeFile : IDisposable
    {
        ...
        private void Open(string path) {
            if (_file != null) throw new InvalidOperationException($"{nameof(NativeFile)}:Open - File is already open!");
            if (_disposed) throw new ObjectDisposedException(nameof(NativeFile), $"{nameof(NativeFile)}:Open - Cannot reopen a disposed {nameof(NativeFile)}, create a new instance.");
            if (string.IsNullOrWhiteSpace(path)) throw new ArgumentOutOfRangeException($"{nameof(NativeFile)}:Open - Empty or Null Path");
        ...
        public int Read(long position, IntPtr buffer, int requestedBytes) {
            ...
            if (!IsOpen()) throw new InvalidOperationException($"{nameof(NativeFile)}:Read - Cannot read, file is not open.");
            if (position + requestedBytes > Length) throw new InvalidOperationException($"{nameof(NativeFile)}:Read - Cannot read from position past end of file");
        ...
        public static void CreateFile(string path, long length, bool replace) {
            ...
            if (fileInfo.Exists && !replace) throw new InvalidOperationException($"NativeFile:CreateFile - File Exists");
