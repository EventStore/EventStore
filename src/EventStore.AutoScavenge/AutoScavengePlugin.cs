...
public class AutoScavengePlugin : AutoScavengePluginBase, IConnectedSubsystemsPlugin {
	...
	public override (bool Enabled, string EnableInstructions) IsEnabled(IConfiguration configuration) {
		var enabledOption = configuration.GetValue<bool?>($"{ConfigurationConstants.Prefix}:AutoScavenge:Enabled");
		var devMode = configuration.GetValue($"{ConfigurationConstants.Prefix}:Dev", defaultValue: false);
		var memDb = configuration.GetValue($"{ConfigurationConstants.Prefix}:MemDb", defaultValue: false);

		// enabled by default
		bool enabled = enabledOption ?? true;

		if (!enabled)
			return (false, $"To enable AutoScavenge: Do not set '{ConfigurationConstants.Prefix}:AutoScavenge:Enabled' to 'false'");
	...
	public override void ConfigureServices(IServiceCollection services, IConfiguration configuration) {
		_options = configuration.GetSection(ConfigurationConstants.Prefix).Get<EventStoreOptions>() ?? _options;
