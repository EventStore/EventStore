...
public class ProjectionEventsProcessedMetricTests : IDisposable {
	...
	private (ProjectionEventsProcessedMetric, TestMeterListener<long>) GenSut(
		[CallerMemberName] string callerName = "") {

		var meter = new Meter($"{typeof(ProjectionEventsProcessedMetricTests)}-{callerName}").DisposeWith(_disposables);
		var listener = new TestMeterListener<long>(meter).DisposeWith(_disposables);
		var sut = new ProjectionEventsProcessedMetric(meter, "test-metric");
		return (sut, listener);
	}
	...
	public void observe_metrics_data() {
		...
		AssertMeasurements(listener,
			AssertMeasurement("TestProjection", 50));
	}
