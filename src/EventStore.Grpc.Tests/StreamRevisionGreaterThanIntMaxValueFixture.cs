...
namespace EventStore.Grpc.Tests {
	public abstract class StreamRevisionGreaterThanIntMaxValueFixture : EventStoreGrpcFixture {
		...
		protected StreamRevisionGreaterThanIntMaxValueFixture(Action<ICheckpoint, TFChunkWriter> runScenario,
			bool external = false) : base(builder => builder.RunOnDisk(CreateExistingDatabase(runScenario)),
			external: external) {
		}
